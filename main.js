...
function animate() {
  ...
  if (puzzle) {
    ...
    const playerPos = cameraController.getPlayer().position;
    if (puzzle.button) {
      puzzle.button.update(playerPos, puzzle.cube ? puzzle.cube.mesh : null);
    }
    if (puzzle.ambientButton) {
      puzzle.ambientButton.update(playerPos, puzzle.cube ? puzzle.cube.mesh : null);
    }
